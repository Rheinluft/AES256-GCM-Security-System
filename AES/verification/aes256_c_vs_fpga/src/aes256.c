#include "aes256.h"

#include <stddef.h>
#include <string.h>

static const uint8_t sbox[256] = {
    
};

static uint8_t xtime(uint8_t value)
{
    return (uint8_t)((value << 1) ^ ((value & 0x80u) ? 0x1bu : 0u));
}

static void add_round_key(uint8_t state[16], const uint8_t round_key[16])
{
    size_t i;

    for (i = 0; i < 16; ++i)
        state[i] ^= round_key[i];
}

static void sub_bytes(uint8_t state[16])
{
    size_t i;

    for (i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

static void shift_rows(uint8_t state[16])
{
    uint8_t shifted[16];

    shifted[0]  = state[0];
    shifted[1]  = state[5];
    shifted[2]  = state[10];
    shifted[3]  = state[15];
    shifted[4]  = state[4];
    shifted[5]  = state[9];
    shifted[6]  = state[14];
    shifted[7]  = state[3];
    shifted[8]  = state[8];
    shifted[9]  = state[13];
    shifted[10] = state[2];
    shifted[11] = state[7];
    shifted[12] = state[12];
    shifted[13] = state[1];
    shifted[14] = state[6];
    shifted[15] = state[11];
    memcpy(state, shifted, sizeof shifted);
}

static void mix_columns(uint8_t state[16])
{
    size_t column;

    for (column = 0; column < 4; ++column) {
        uint8_t *s = state + column * 4;
        uint8_t a0 = s[0];
        uint8_t a1 = s[1];
        uint8_t a2 = s[2];
        uint8_t a3 = s[3];
        uint8_t total = (uint8_t)(a0 ^ a1 ^ a2 ^ a3);

        s[0] ^= (uint8_t)(total ^ xtime((uint8_t)(a0 ^ a1)));
        s[1] ^= (uint8_t)(total ^ xtime((uint8_t)(a1 ^ a2)));
        s[2] ^= (uint8_t)(total ^ xtime((uint8_t)(a2 ^ a3)));
        s[3] ^= (uint8_t)(total ^ xtime((uint8_t)(a3 ^ a0)));
    }
}

void aes256_expand_key(const uint8_t key[32], uint8_t round_keys[240])
{
    size_t generated = 32;
    uint8_t rcon = 1;
    uint8_t temp[4];
    size_t i;

    memcpy(round_keys, key, 32);
    while (generated < 240) {
        memcpy(temp, round_keys + generated - 4, sizeof temp);
        if (generated % 32 == 0) {
            uint8_t first = temp[0];
            temp[0] = (uint8_t)(sbox[temp[1]] ^ rcon);
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[first];
            rcon = xtime(rcon);
        } else if (generated % 32 == 16) {
            for (i = 0; i < 4; ++i)
                temp[i] = sbox[temp[i]];
        }
        for (i = 0; i < 4 && generated < 240; ++i) {
            round_keys[generated] =
                (uint8_t)(round_keys[generated - 32] ^ temp[i]);
            ++generated;
        }
    }
}

void aes256_encrypt_block(const uint8_t plaintext[16], uint8_t ciphertext[16],
                          const uint8_t round_keys[240])
{
    uint8_t state[16];
    unsigned round;

    memcpy(state, plaintext, sizeof state);
    add_round_key(state, round_keys);
    for (round = 1; round < 14; ++round) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys + round * 16u);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys + 14u * 16u);
    memcpy(ciphertext, state, sizeof state);
}
