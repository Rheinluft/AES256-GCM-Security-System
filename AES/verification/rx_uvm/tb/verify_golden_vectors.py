#!/usr/bin/env python3
"""Independently validate the OpenSSL-generated RX golden vector handoff.

Two checks, both must pass before the vectors are trusted as UVM stimulus:
  1. SHA-256 of every file matches manifest.txt.
  2. Re-encrypting plaintext with an independent AES-GCM implementation
     (python cryptography) reproduces ciphertext_1280.bin and tag_1280.bin
...
HANDOFF = Path(__file__).resolve().parents[2] / \
    "00_1_benchmark" / "rx_core_golden_handoff_pcam_1280_20260813"
...
for line in (HANDOFF / "manifest.txt").read_text().splitlines()[1:]:
...
aad_all = (HANDOFF / "aad_1280.bin").read_bytes()
iv_all = (HANDOFF / "iv_1280.bin").read_bytes()
pt_all = (HANDOFF / "plaintext_1280.bin").read_bytes()
ct_all = (HANDOFF / "ciphertext_1280.bin").read_bytes()
tag_all = (HANDOFF / "tag_1280.bin").read_bytes()
