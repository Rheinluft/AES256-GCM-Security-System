...
$Xsdb = Get-Setting $Xsdb @('AES_GCM_TX_XSDB', 'AES_GCM_XSDB') ''
$CableSerial = Get-Setting $CableSerial @('AES_GCM_TX_CABLE_SERIAL', 'AES_GCM_JTAG_CABLE_SERIAL') '210351BE7DF5A'
...
try {
    ...
    Write-Host "TX JTAG: cable=$CableSerial port=$Port; Linux discovers the physical wired video NIC"
    & $xsdbPath $tcl
